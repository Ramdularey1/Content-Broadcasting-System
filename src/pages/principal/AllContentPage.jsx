...
export function AllContentPage() {
  ...
  const rows = useMemo(() => {
    ...
    return content.filter((item) => {
      ...
      const matchesQuery = !query || [item.title, item.subject, item.teacherName].some((value) => value.toLowerCase().includes(query));
      return matchesStatus && matchesQuery;
    });
  }, [content, search, status]);

  return (
    <AppLayout title="All Content" subtitle="Search, filter, and audit all uploaded content across the school.">
      <section className="section">
        ...
        {loading ? <SkeletonList rows={9} /> : rows.length === 0 ? <EmptyState /> : <ContentTable rows={rows} page={page} onPageChange={setPage} onView={setSelected} />}
      </section>
      {selected ? (
